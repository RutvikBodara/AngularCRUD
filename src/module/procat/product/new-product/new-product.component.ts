import {ChangeDetectionStrategy, Component, computed,signal, input, viewChild, ViewChild, ElementRef} from '@angular/core';
...
export class NewProductComponent {
  ...
  ngOnInit(){
    ...
    this.productForm = this.fb.group({
      ...
      helplineNumber:['',[Validators.required,Validators.minLength(10),Validators.maxLength(10)]],
  ...
  onSubmit()
  {
    ...
    if(this.productForm.valid){
      ...
      const formattedDate = this.datePipe.transform(data.launchDate, 'dd-MM-YYYY');
      const formValue = this.productForm.value;
      console.log(formValue)
      const checkedSubtasks = formValue.countries.filter((subtask: any) => subtask.completed);
      const lastFormattedDate =this.datePipe.transform(data.lastDate,'dd-MM-YYYY');
  ...
  addProduct(data){
    console.log(data);
    console.log("finals")
    this.componentService.add<string>(data,APIURL.AddProduct).subscribe((res)=>{
     if(res.code == 100){
