...
export class ManipulateProductComponent {
      ...
      if (currentTask.subtasks) {
        this.countryList.forEach((subtask) => {
          ...
          currentTask.subtasks.push({
            ...
            name: subtask.name,
            completed:
              checksubtask != null && checksubtask.includes(subtask.id)
                ? true
                : false,
          });
        });
  ...
  ngOnInit() {
    ...
    this.productSubscription= this.productSubscription = this.commonService.product$.subscribe((value: product) => {
    ...
    this.productForm = this.fb.group({
      ...
      name: [
        this.currentProduct != null ? this.currentProduct.name : '',
        [Validators.required, Validators.minLength(2)],
      ],
      description: [
        this.currentProduct != null ? this.currentProduct.description : '',
      ...
      launchDate: [
        this.currentProduct != null
          ? this.stringToDate(this.currentProduct.launchDate)
          : '',
        [Validators.required],
      ],
      lastDate: [
        this.currentProduct != null
          ? this.stringToDate(this.currentProduct.lastDate)
          : '',
        [Validators.required],
      ],
      file: [this.urlInstance],
      price: [
        this.currentProduct != null ? this.currentProduct.price : '',
        [Validators.required],
      ],
      availableForSale: [
        this.currentProduct != null &&
        this.currentProduct.availableForSale == true
          ? 'true'
          : 'false',
        [Validators.required],
      ],
      helplineNumber: [
        this.currentProduct != null ? this.currentProduct.helplineNumber : '',
        [
          Validators.required,
  ...
  async onSubmit() {
    ...
    const formattedLaunchDate = this.datePipe.transform(
      data.launchDate,
      'dd-MM-YYYY'
    );
    ...
    const checkedSubtasks = data.countries.filter(
    ...
    if (data.lastDate < data.launchDate) {
      this.commonService.showSnackBar(
        'last date must be greater than launch date'
      );
      return false;
    }
    if (this.productForm.valid) {
      const base64File: string = this.file
        ? await this.getBase64(this.file)
        : null;
      this.requestData = {
        id: data.id,
        name: data.name,
  ...
  postEditProduct() {
    this.componentService
      .updateProduct(this.requestData, APIURL.editProduct)
      .subscribe((res) => {
        if (res.code == 100) {
          this.router.navigate(['contact/product']);
